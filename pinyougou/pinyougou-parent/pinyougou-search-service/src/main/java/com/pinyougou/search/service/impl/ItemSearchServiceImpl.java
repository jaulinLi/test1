...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    public Map<String, Object> search(Map searchMap) {
        ...
        if (searchMap != null) {
            ...
            if (StringUtils.isNotBlank(price)) {
                ...
                if (ranges != null && ranges.length == 2) {
                    criteria = new Criteria("item_price")
                            .between(Long.parseLong(ranges[0]),
                ...
                ranges = price.split(" ");
                if (ranges != null && ranges.length == 2) {
                    criteria = new Criteria("item_price").greaterThanEqual(ranges[0]);
                }
                FilterQuery filterQuery = new SimpleFilterQuery(criteria);
                query.addFilterQuery(filterQuery);
            }
            ...
            if (StringUtils.isNotBlank(sort) && StringUtils.isNotBlank(sortFiled)) {
                Sort orders = null;
                if (sort.equalsIgnoreCase("ASC")) {
                    orders = new Sort(Sort.Direction.ASC, sortFiled);
                } else {
                    orders = new Sort(Sort.Direction.DESC, sortFiled);
                }
    ...
    public Map<String, Object> getBrandAndSpec(String category) {
        ...
        if (typeTemplateId != null) {
            ...
            //获取规格信息
            List<Map> specList = (List<Map>) redisTemplate.boundHashOps("SpcList").get(typeTemplateId);
